...
核心参数:
  --stage_schedule TEXT      阶段 schedule，例如 '0.875-0.625:50,0.75-0.5:50'
  --target_update_steps N    需要的有效更新步数；不传默认等于 schedule 步数之和
...
常用训练参数:
  ...
  --dataset_path PATH
  --scores_file PATH
  --shuffle_train_scores      按 seed 打乱 --scores_file 的行顺序
  --train_scores_shuffle_seed N  只控制 --scores_file 行打乱的 seed
  --start_uid1 N
  --end_uid1 N
  --output_dir PATH
  --batch_size N
...
示例:
  bash run_train_accuracy_gate_schedule.sh --stage_schedule '0.875-0.625:50,0.75-0.5:50'
  bash run_train_accuracy_gate_schedule.sh --stage_schedule '0.875-0.625:50,0.75-0.5:50' ./grpo_accuracy_gate/checkpoint-50
EOF
}
...
SHUFFLE_TRAIN_SCORES="${SHUFFLE_TRAIN_SCORES:-0}"
TRAIN_SCORES_SHUFFLE_SEED="${TRAIN_SCORES_SHUFFLE_SEED:-}"
MIN_UID1="${MIN_UID1:-}"
MAX_UID1="${MAX_UID1:-}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --shuffle_train_scores)
      SHUFFLE_TRAIN_SCORES=1
      shift
      ;;
    --train_scores_shuffle_seed)
      TRAIN_SCORES_SHUFFLE_SEED="$2"
      shift 2
      ;;
    --min_uid1|--start_uid1)
      MIN_UID1="$2"
    ...
    --max_attempt_steps)
      ...
      ;;
    --min_correct_rate)
      echo "Error: --min_correct_rate is not used by scheduled training; use --stage_schedule instead." >&2
      exit 1
      ;;
    --max_correct_rate)
      echo "Error: --max_correct_rate is not used by scheduled training; use --stage_schedule instead." >&2
      exit 1
      ;;
    --batch_size)
...
if [ -z "${STAGE_SCHEDULE}" ]; then
  echo "Error: --stage_schedule is required, e.g. '0.875-0.625:50,0.75-0.5:50'" >&2
  exit 1
fi
...
SHUFFLE_TRAIN_SCORES_ARG=()
if [ "${SHUFFLE_TRAIN_SCORES}" = "1" ]; then
  SHUFFLE_TRAIN_SCORES_ARG=(--shuffle_train_scores)
fi
TRAIN_SCORES_SHUFFLE_SEED_ARG=()
if [ -n "${TRAIN_SCORES_SHUFFLE_SEED}" ]; then
  TRAIN_SCORES_SHUFFLE_SEED_ARG=(--train_scores_shuffle_seed "${TRAIN_SCORES_SHUFFLE_SEED}")
fi
MIN_UID1_ARG=()
if [ -n "${MIN_UID1}" ]; then
...
TARGET_UPDATE_ARG=()
if [ -n "${TARGET_UPDATE_STEPS}" ]; then
  TARGET_UPDATE_ARG=(--target_update_steps "${TARGET_UPDATE_STEPS}")
fi
...
CMD=(
  ...
  train_grpo_accuracy_gate_schedule.py
...
print_summary() {
  echo "==== GRPO Scheduled Accuracy-Gated Training Start ===="
  echo "Time: ${TIME}"
  echo "Log: ${LOG_FILE}"
  echo "Mode: ${RUN_MODE}"
  ...
  echo "Train scores file: ${TRAIN_SCORES_FILE:-<none>}"
  echo "Shuffle train scores: ${SHUFFLE_TRAIN_SCORES}"
  echo "Train scores shuffle seed: ${TRAIN_SCORES_SHUFFLE_SEED:-<seed>}"
  echo "Accepted length interval: [${ACCEPTED_LENGTH_MIN}, ${ACCEPTED_LENGTH_MAX:-disabled}]"
  echo "Target accepted update steps: ${TARGET_UPDATE_STEPS:-schedule_total}"
...
if [ "${RUN_MODE}" = "foreground" ]; then
  ...
  echo "==== Scheduled Accuracy-Gated Training Finished ===="
  echo "Output dir: ${OUTPUT_DIR}"
  exit 0
fi
