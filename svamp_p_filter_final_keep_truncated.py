...
from dataset_utils import SUPPORTED_PROMPT_STYLES, build_messages
...
from svamp_p_filter import build_scored_row
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Final SVAMP p scorer that keeps scored rows even when generations are length-truncated."
    )
    parser.add_argument("--base_model", type=str, default="Qwen/Qwen2.5-0.5B-Instruct")
    parser.add_argument("--adapter_path", type=str, default=None)
    parser.add_argument("--dataset_path", type=str, default="data/svamp-train.arrow")
    parser.add_argument("--split", type=str, default="train")
    parser.add_argument("--out", type=str, default="outputs/SVAMP/svamp_p_scores_final_keep_truncated.jsonl")
...
def main():
    ...
    log("Final keep-truncated SVAMP run config:")
    log(f"  dataset=svamp")
    ...
    with open(args.out, out_mode, encoding="utf-8") as handle:
        progress = tqdm(total=len(target_uid_set), desc="Final scoring SVAMP", dynamic_ncols=True)
        if completed_uids:
