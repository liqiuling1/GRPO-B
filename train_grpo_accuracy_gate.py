...
def parse_args():
    ...
    parser.add_argument("--target_update_steps", type=int, default=50)
...
def main():
    ...
    if not 0.0 <= args.min_correct_rate <= 1.0 or not 0.0 <= args.max_correct_rate <= 1.0:
        raise ValueError("--min_correct_rate and --max_correct_rate must be within [0, 1].")
    if args.min_correct_rate > args.max_correct_rate:
        raise ValueError("--min_correct_rate cannot be greater than --max_correct_rate.")
    ...
    print(
        "Accuracy gate: "
        f"min_correct_rate={args.min_correct_rate}, max_correct_rate={args.max_correct_rate}, "
    ...
    trainer = AccuracyGateGRPOTrainer(
        model=model,
        args=training_args,
