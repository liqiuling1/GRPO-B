...
from dataset_utils import GSM8K_COT_STOP_STRINGS, SUPPORTED_PROMPT_STYLES, build_messages
from evaluate_svamp import get_svamp_answer, get_svamp_question, load_svamp_dataset
from gsm8k_p_filter import (
    build_truncation_row,
    compute_p,
    derive_truncation_out_path,
    load_jsonl_rows,
    load_uid_filter,
    normalize_uid_range,
...
def build_scored_row(
    ...
    return {
        ...
        "split": split,
        "dataset": "svamp",
        "svamp_id": str(example.get("ID", example.get("svamp_id", ""))),
        "svamp_type": str(example.get("Type", example.get("svamp_type", ""))),
        "question": question,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Probability-only SVAMP scorer using the GSM8K p-filter output format."
    )
    parser.add_argument("--base_model", type=str, default="Qwen/Qwen2.5-0.5B-Instruct")
    parser.add_argument("--adapter_path", type=str, default=None)
    parser.add_argument("--dataset_path", type=str, default="data/svamp-train.arrow")
    parser.add_argument("--split", type=str, default="train")
    parser.add_argument("--out", type=str, default="outputs/svamp_p_scores.jsonl")
...
def main():
    ...
    log("Run config:")
    log(f"  dataset=svamp")
    log(f"  dataset_path={args.dataset_path or '<huggingface>'}")
    ...
    if args.resume:
        existing_rows = [
            row for row in load_jsonl_rows(args.out) if str(row.get("uid")) in target_uid_set
        ]
    ...
    with open(args.out, out_mode, encoding="utf-8") as handle, open(
        truncation_out, truncation_mode, encoding="utf-8"
    ) as truncation_handle:
        progress = tqdm(total=len(target_uid_set), desc="Scoring SVAMP", dynamic_ncols=True)
        if completed_uids:
        ...
        for batch_start in range(0, len(dataset), max(1, args.prompt_batch_size)):
            ...
            questions = [get_svamp_question(example) for example in batch_examples]
            gold_answers = [get_svamp_answer(example) for example in batch_examples]
            ...
            for local_idx in pending_local_indices:
                ...
                row = build_scored_row(
                    ...
                    example=batch_examples[local_idx],
                )
                ...
                truncation_row = build_truncation_row(row=row, answer_infos=answer_infos)
        ...
        summary.update(
            {
                ...
                "warning": "Resumed from an older output file without per-sample diagnostics.",
            }
        )
